...
resource "github_repository_webhook" "aws-pdm-dataset-generation" {
  ...
  configuration {
    url          = "https://${var.aws_concourse_domain_name}/api/v1/teams/${var.aws_concourse_team}/pipelines/pdm-dataset-generation/resources/${github_repository.aws-pdm-dataset-generation.name}/check/webhook?webhook_token=${var.github_webhook_token}"
    content_type = "form"
  }
}
...
resource "github_repository_webhook" "aws-pdm-dataset-generation_pr" {
  ...
  configuration {
    url          = "https://${var.aws_concourse_domain_name}/api/v1/teams/${var.aws_concourse_team}/pipelines/pdm-dataset-generation/resources/${github_repository.aws-pdm-dataset-generation.name}-pr/check/webhook?webhook_token=${var.github_webhook_token}"
    content_type = "form"
  }
}
