...
resource "github_repository_webhook" "dataworks_internet_ingress" {
  ...
  configuration {
    url          = "https://${var.aws_concourse_domain_name}/api/v1/teams/${var.aws_concourse_team}/pipelines/${github_repository.dataworks_internet_ingress.name}/resources/${github_repository.dataworks_internet_ingress.name}/check/webhook?webhook_token=${var.github_webhook_token}"
    content_type = "form"
  }
}
...
resource "github_repository_webhook" "dataworks_internet_ingress_pr" {
  ...
  configuration {
    url          = "https://${var.aws_concourse_domain_name}/api/v1/teams/${var.aws_concourse_team}/pipelines/${github_repository.dataworks_internet_ingress.name}/resources/${github_repository.dataworks_internet_ingress.name}-pr/check/webhook?webhook_token=${var.github_webhook_token}"
    content_type = "form"
  }
}
