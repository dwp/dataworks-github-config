...
resource "github_repository_webhook" "dataworks-github-config" {
  ...
  configuration {
    url          = "https://${var.aws_concourse_domain_name}/api/v1/teams/${var.aws_concourse_team}/pipelines/${github_repository.dataworks-github-config.name}/resources/${github_repository.dataworks-github-config.name}/check/webhook?webhook_token=${var.github_webhook_token}"
    content_type = "form"
  }
}
...
resource "github_repository_webhook" "dataworks-github-config-pr" {
  ...
  configuration {
    url          = "https://${var.aws_concourse_domain_name}/api/v1/teams/${var.aws_concourse_team}/pipelines/${github_repository.dataworks-github-config.name}/resources/${github_repository.dataworks-github-config.name}-pr/check/webhook?webhook_token=${var.github_webhook_token}"
    content_type = "form"
  }
}
