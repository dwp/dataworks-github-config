locals {
  pipeline_name = "dataworks_ml_streams_infrastructure"
}
...
resource "github_repository_webhook" "dataworks_ml_streams_infrastructure" {
  ...
  configuration {
    url          = "https://${var.aws_concourse_domain_name}/api/v1/teams/${var.aws_concourse_team}/pipelines/${local.pipeline_name}/resources/${github_repository.dataworks_ml_streams_infrastructure.name}/check/webhook?webhook_token=${var.github_webhook_token}"
    content_type = "form"
  }
}
...
resource "github_repository_webhook" "dataworks_ml_streams_infrastructure_pr" {
  ...
  configuration {
    url          = "https://${var.aws_concourse_domain_name}/api/v1/teams/${var.aws_concourse_team}/pipelines/${local.pipeline_name}/resources/${github_repository.dataworks_ml_streams_infrastructure.name}-pr/check/webhook?webhook_token=${var.github_webhook_token}"
    content_type = "form"
  }
}
